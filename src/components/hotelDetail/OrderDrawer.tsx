...
const OrderDrawer = ({
  ...
  const calculateGrandTotal = () => {
    ...
    if (tableNumber === 0) {
      grandTotal += computeParcelCharge(hotelData?.delivery_rules, items);
    }
              ...
              <div className="mx-auto flex max-w-2xl items-center justify-between rounded-xl bg-emerald-600 px-4 py-3 text-white shadow-xl shadow-emerald-600/25 transition active:scale-[0.99]">
                <div className="flex items-center gap-2.5">
                  ...
                  <span className="text-sm font-bold">
                    {totalQty} item{totalQty > 1 ? "s" : ""} · <MenuPrice currency={hotelData?.currency || "₹"} amount={totalPrice.toFixed(2)} />
                  </span>
                </div>
        ...
        return (
          ...
          >
            <span className="font-semibold text-sm whitespace-nowrap">
              {totalQty} {totalQty === 1 ? "item" : "items"} | <MenuPrice currency={hotelData?.currency || "₹"} amount={totalPrice.toFixed(2)} />
            </span>
