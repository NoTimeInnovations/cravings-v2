...
function V4BottomSheet({ onClose, children }: { onClose: () => void; children: React.ReactNode }) {
...
const V4ItemCard = ({
  ...
  return (
    <>
      ...
      <div
        ref={inViewRef}
        className="flex gap-3 py-3.5 cursor-pointer transition-all duration-500 ease-out"
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <h3 className="mt-1.5 text-[15px] font-bold leading-snug text-gray-900 text-left" dir="auto">
                ...
                hasValidMainOffer ? (
                  <div className="flex items-center gap-1.5">
                    ...
                    {!hasMultipleVariantsOnOffer && hasValidMainOriginalPrice && (
                      <span className="text-[11px] line-through opacity-40 font-normal">
                  ...
                  <span className="text-gray-900">
                    {baseItemPrice > 0 ? (
                      <>
                        {hasVariants && <span className="text-[11px] font-normal">From </span>}
          ...
          {/* Description */}
          {item.description && (
            <p className="mt-1.5 line-clamp-2 whitespace-pre-line text-[12px] leading-relaxed text-gray-400">
              {item.description}
            </p>
          )}
        </div>
        ...
        {/* Right - Image + Add button.
            The box size is pinned with INLINE styles (not Tailwind arbitrary
            classes) so every thumbnail is the exact same fixed size regardless
            of the source image's natural dimensions — object-cover crops to
        ...
        <div className="flex shrink-0 flex-col items-center">
          ...
          {/* Add button overlay — bottom-center of the image (same logic as V3).
              stopPropagation keeps it from also opening the detail sheet. */}
          {isOrderable && (
            <div className="absolute -bottom-3 left-1/2 -translate-x-1/2">
              {offerData && item.category?.name?.toLowerCase() === "custom" ? (
                <button
                  onClick={(e) => { e.stopPropagation(); router.push(`/offers/${offerData.id}`); }}
                  className="rounded-lg border border-emerald-600/30 bg-white px-5 py-1.5 text-[11px] font-extrabold uppercase tracking-wider text-emerald-700 shadow-md transition active:scale-95"
                >
                  View
                </button>
                ...
                !defaultShowOptions ? (
                  (!hasOrderingFeature && !hasDeliveryFeature) ? (
                    ...
                    <div className="flex items-center gap-0.5 rounded-lg border border-emerald-600/30 bg-white px-1 py-1 shadow-md">
                      ...
                      <span className="min-w-[18px] text-center text-xs font-extrabold text-emerald-700">{itemQuantity}</span>
                      <button
                        onClick={(e) => { e.stopPropagation(); setShowVariants(true); }}
                        className="flex h-6 w-6 items-center justify-center rounded text-emerald-700"
                      >
      ...
      {showItemSheet && !hasVariants && typeof window !== "undefined" && createPortal(
        <V4BottomSheet onClose={() => setShowItemSheet(false)}>
      ...
      {showVariants && hasVariants && typeof window !== "undefined" && createPortal(
        <V4BottomSheet onClose={() => setShowVariants(false)}>
          <div className="sticky top-0 z-10 flex justify-center bg-white pt-2.5 pb-1">
            <div className="h-1 w-8 rounded-full bg-gray-200" />
          </div>
