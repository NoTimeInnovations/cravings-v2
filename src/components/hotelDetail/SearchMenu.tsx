...
const SearchMenu = ({
  ...
  const [variantModalItem, setVariantModalItem] =
    useState<HotelDataMenus | null>(null);
  const [selectedVariant, setSelectedVariant] = useState<any>(null);
  const [expandedVariantItemId, setExpandedVariantItemId] = useState<
    string | null
  >(null);
  ...
  useEffect(() => {
    if (open_place_order_modal && isOpen) {
      handleClose();
    }
  }, [open_place_order_modal]);

  // Close search when OrderDrawer's "View Order" is clicked (covers non-logged-in flow too)
  useEffect(() => {
    const onCloseSearch = () => {
      if (isOpen) handleClose();
    };
    window.addEventListener("close-search-overlay", onCloseSearch);
    return () =>
      window.removeEventListener("close-search-overlay", onCloseSearch);
  }, [isOpen]);
          ...
          >
            <div className="flex items-center gap-3">
              <div
                ...
                style={{
                  backgroundColor: `${styles.color}08`,
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex items-center">
                        {/* Image */}
                        <div
                          className="w-20 h-20 relative overflow-hidden flex-shrink-0 m-2.5 rounded-lg"
                          style={{ backgroundColor: `${styles.color}0D` }}
                        >
                          {item.image_url ? (
                            <img
                              src={item.image_url}
                        ...
                        <div className="flex-1 py-2.5 pl-5 pr-3 flex flex-col">
                          <div className="flex justify-between items-start">
                            <div className="flex-1 min-w-0 pr-3">
                              <h3
                                className="text-sm font-bold capitalize mb-1 line-clamp-2"
                                style={{ color: styles.color }}
                              >
                                {item.name}
                              </h3>
                              ...
                              <div className="flex flex-col items-end gap-2">
                                {!hasVariants ? (
                                  quantity === 0 ? (
                                    ...
                                    <div
                                      className="flex items-center gap-1.5 text-white rounded-lg px-2 py-1.5"
                                      style={{ backgroundColor: styles.accent }}
                                    >
                                      <button
                                        onClick={() =>
                      ...
                      {hasVariants && isExpanded && (
                        <div
                          ...
                          style={{
                            borderTop: `1px solid ${styles.border.borderColor}`,
                          }}
                        >
                          <h4
                                ...
                                {showAddButton &&
                                  ...
                                  0 ? (
                                    ...
                                    >
                                      <button
                                        onClick={() =>
                                          handleVariantRemove(item, variant)
                                        }
                                        className="w-5 h-5 flex items-center justify-center rounded-full active:opacity-70"
                                      >
                                        <Minus size={12} />
