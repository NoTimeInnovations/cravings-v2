...
import {
  CompactMenuPreview,
  ColorPalette,
} from "@/components/get-started/CompactMenuPreview";
...
const SAMPLE_MENU_ITEMS: MenuItem[] = [
  ...
  },
  {
    ...
    price: 3.25,
    description:
      "Refreshing, freshly squeezed fruit juices for a vibrant start.",
    category: "beverages",
  },
  {
    name: "Tea",
  ...
  },
  {
    ...
    price: 9.5,
    description:
      "Premium organic sirloin beef for the ultimate burger experience.",
    category: "burgers",
  },
  {
  ...
  },
  {
    ...
    price: 7.25,
    description:
      "Savory roasted tofu stuffed with a flavorful vegetable medley.",
    category: "entrées",
  },
  {
  ...
  <div className="flex flex-col gap-1.5">
    ...
    <Popover>
      <PopoverTrigger asChild>
        ...
        >
          <span
            className={`text-xs font-mono px-1.5 py-0.5 rounded bg-white/20 backdrop-blur-md ${isColorDark(color) ? "text-white" : "text-black"}`}
          >
            {color}
          </span>
        </button>
...
export default function GetStartedClient({
  ...
  const [mobileTab, setMobileTab] = useState<"background" | "text" | "accent">(
    "accent",
  );
  ...
  const handleFinalPublish = async () => {
    ...
    try {
      ...
      const uniqueCategories = Array.from(
        new Set(extractedItems.map((i) => i.category)),
      );
      const categoriesData = uniqueCategories.reduce(
        (acc, catName, index) => {
          acc[catName] = {
            name: catName,
            priority: index,
  ...
  const handleEmailChange = async () => {
    ...
    try {
      const { updateEmailAndResend } =
        await import("@/app/actions/updateEmailAndResend");
  ...
  const renderStep3 = () => {
    ...
    return (
      <div className="max-w-6xl mx-auto flex flex-col md:flex-row md:items-start gap-8 animate-in fade-in duration-700 md:pt-12 pb-24 md:pb-0">
        <div
          className={`flex-1 space-y-6 ${registrationSuccess ? "hidden md:block" : ""}`}
        >
          {registrationSuccess ? (
            renderSuccessView()
          ) : (
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col">
      ...
      {/* Main Content */}
      <main
        className={`max-w-6xl mx-auto ${step === 3 ? "px-0 py-0 md:px-6" : step === 2 ? "px-6 py-6 sm:py-8" : "px-6 py-12"}`}
      >
        {step === 1 && renderStep1()}
        {step === 2 && renderStep2()}
        {step === 3 && renderStep3()}
