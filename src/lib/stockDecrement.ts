...
// Re-enable a global-stock item that a restock lifted back above 0 (inverse of
// DISABLE_MENU_ITEM). Only applied on the 0-crossing so it can't un-hide an item
// the partner disabled while still in stock.
const ENABLE_MENU_ITEM = `
  mutation EnableMenuItem($id: uuid!) {
    update_menu_by_pk(pk_columns: { id: $id }, _set: { is_available: true }) {
      id
    }
  }
`;

// Atomically CLAIM an order's stock: false -> true. Returns affected_rows so the
// caller decrements ONLY when it won the claim (affected_rows === 1), making
// decrement exactly-once even under placement retries. Also stamps the exact
// stock_date bucket so restock can re-hit the same menu_date_stocks row.
const CLAIM_STOCK = `
  mutation ClaimOrderStock($id: uuid!, $date: date!) {
    update_orders(
      where: { id: { _eq: $id }, stock_committed: { _eq: false } }
      _set: { stock_committed: true, stock_date: $date }
    ) {
      affected_rows
    }
  }
...
// Atomically claim an order's stock before decrementing. Returns true only if
// this call flipped stock_committed false -> true (so the caller should run the
// decrement). A losing/duplicate claim returns false and must NOT decrement.
export async function claimOrderStock(orderId: string, stockDate: string): Promise<boolean> {
  try {
    const res = await fetchFromHasura(CLAIM_STOCK, { id: orderId, date: stockDate });
    return (res?.update_orders?.affected_rows ?? 0) === 1;
  } catch (e) {
    console.error("[stockDecrement] claim failed", orderId, e);
    return false;
  }
}

// Exact inverse of decrementStockForOrder: add the ordered quantities back.
// Routing is re-resolved from the DB (never trusts the caller). Date-capped
// items increment the (menu, stockDate) row IF it exists (never seed/fabricate);
// global items increment stocks.stock_quantity and re-enable the menu item only
// on the 0-crossing. Callers MUST pass the same stockDate the order decremented.
// Best-effort per item; never throws. Idempotency is the caller's job (gate the
// whole restock behind an atomic RELEASE of stock_committed).
export async function incrementStockForOrder(
  ...
  try {
    ...
    console.error("[stockDecrement] restock: failed to load stock rows", e);
    return;
  }
  const stockByMenu = new Map<string, { stockId: string; dailyDefault: number | null }>();
  for (const r of rows) {
  ...
  for (const [menuId, qty] of qtyByMenu) {
    ...
    try {
      if (info.dailyDefault != null) {
        // Date-capped: add back to the same date bucket. If the row doesn't
        // exist (nothing was decremented for that date), skip — never fabricate.
        await fetchFromHasura(DECREASE_DATE_STOCK, { menuId, date: stockDate, by: qty });
      } else {
        // Global: add back, and re-enable only if this lifted it back above 0.
        const res = await fetchFromHasura(DECREASE_STOCK, { stockId: info.stockId, by: qty });
        const newQty = res?.update_stocks_by_pk?.stock_quantity;
        if (typeof newQty === "number" && newQty - qty <= 0 && newQty > 0) {
          await fetchFromHasura(ENABLE_MENU_ITEM, { id: menuId });
        }
      }
    } catch (e) {
      console.error("[stockDecrement] restock failed for", menuId, stockDate, e);
    }
  }
}
