...
import type { VariableMapItem } from "@/lib/whatsapp-broadcast";

/**
 * WhatsApp broadcast worker. Each run:
 ...
 * /api/cron/dispatch-broadcasts route and by the merged /api/cron/dispatch.
 */
...
export async function runBroadcasts() {
  ...
  const due = (data?.whatsapp_broadcasts || []) as DueBroadcast[];
  summary.due = due.length;

  // No work this tick — skip loading the heavy WhatsApp send deps entirely.
  ...
  const { isWhatsappEnabled } = await import("@/lib/whatsapp-features");
