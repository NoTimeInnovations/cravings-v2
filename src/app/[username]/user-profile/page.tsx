...
export default function RestaurantUserProfilePage() {
  ...
  // The restaurant this profile is scoped to. Resolved from the username param.
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        console.error("Error resolving restaurant for profile:", err);
      } finally {
  ...
  // Prefer the store name/currency from the resolved partner, but fall back to
  // whatever the orders carry (covers the brief window before resolution).
  const displayStoreName =
    storeName || (restaurantOrders[0]?.partner as any)?.store_name || "";
  const displayCurrency =
    storeCurrency || (restaurantOrders[0]?.partner as Partner)?.currency || "₹";
  ...
  const handleLogout = async () => {
    isLoggingOut.current = true;
    await signOut();
    // Stay on the restaurant page the customer was browsing.
    router.replace(restaurantPath);
  };
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="container mx-auto px-4 pb-24 pt-4 max-w-lg">
        ...
        {/* Orders — scoped to this restaurant only */}
        <div>
          <h2 className="text-lg font-semibold mb-4 flex items-center gap-2 text-gray-900">
            <ShoppingBag className="h-5 w-5" />
            My Orders{displayStoreName ? ` · ${displayStoreName}` : ""}
          </h2>
            ...
            <div className="rounded-xl overflow-hidden shadow-sm bg-white border border-gray-200">
              {displayStoreName && (
                <div className="px-4 py-3 border-b border-gray-100 flex items-center justify-between bg-gray-50">
                  <h3 className="font-semibold text-sm text-gray-900">{displayStoreName}</h3>
                ...
                {restaurantOrders.map((order, idx) => {
                  ...
                  return (
                    <Link
                      key={order.id}
                      href={`/order/${order.id}`}
                      className={`px-4 py-3 flex items-center justify-between transition-colors ${idx < restaurantOrders.length - 1 ? "border-b border-gray-100" : ""
                        }`}
                    >
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                      ...
                      <div className="flex items-center gap-2">
                        <span className="text-sm font-semibold text-gray-900">
                          {displayCurrency}{foodTotal.toFixed(2)}
                        </span>
